...
#### 1. Convert mesh files to `.vtk` files
  - Ensure all of your mesh files are in one folder, saved as PLY or STL files.
...
#### 4. Run deformetrica
- Make sure (1) all of your aligned mesh files are in your experiment directory, (2) the mesh files match the files specified in 'data_set.xml', and (3) all 3 xmls are in the experiment directory.
- Navigate into your experiment directory in a terminal and run: 
```
deformetrica estimate model.xml data_set.xml -p optimization_parameters.xml [--output outdirectory]
```
- Model results will be saved by default in an 'output' subdirectory in your experiment folder, please add an output flag to specify directory. 
...
## Other Details
### File Structure
To use the code as formatted, data files are expected to organized in a *data* folder in the following way. The segmentations folder is only needed if using batch conversion to mesh files, and the landmarks folder is only required if using the `align.py` function. Otherwise only a series of mesh files need in the mesh folder. 
